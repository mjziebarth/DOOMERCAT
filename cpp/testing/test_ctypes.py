...
def compute_cost(lon, lat, data_lon, data_lat, k0, pnorm, k0_ap, sigma_k0, f):
    """
    ...
    result = np.ones(1)
    res = _doomercat_so.compute_cost(c_size_t(N), data_lon.ctypes.data_as(POINTER(c_double)),
                                     data_lat.ctypes.data_as(POINTER(c_double)), c_double(lon),
                                     c_double(lat), c_double(k0), c_double(f), c_uint(pnorm),
...
def compute_cost_and_gradient(lon, lat, data_lon, data_lat, k0, pnorm, k0_ap, sigma_k0, f):
    """
