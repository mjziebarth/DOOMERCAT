...
class WktCRS:
    ...
    @staticmethod
    def parse_ellipsoid(geowkt: str) -> tuple[float,float]:
        # Both 'SPHEROID' and 'ELLIPSOID' are allowed, see section
        # '8.1.2 Ellipsoid' of OGC 18-010r11.
        if "ELLIPSOID" in geowkt:
            split = geowkt.split("ELLIPSOID[")
        else:
            split = geowkt.split("SPHEROID[")
        if len(split) != 2:
            raise RuntimeError("Not exactly one ellipsoid keyword given.")

        # Now get rid of the name in double quote. This is a bit tricky
        # since there may be double quotes within the Wkt Latin Text
        # (see section '6.3.5 Double quote' of OGC 18-010r11).
        # Double quote in text is marked by two consecutive double quotes.
        spheroid_part = split[1].replace('""','')

        # Now the name remains with only two double quotes: the delimiters.
        spheroid_part = spheroid_part.split('"')[2]
        a_str, f_inv, arg3 = spheroid_part.split(",")[1:4]

        # Handle possibly given length unit:
        a = float(a_str)
        if arg3.startswith("LENGTHUNIT") or arg3.startswith("UNIT"):
            # We may parse the length unit!
            # Something like 'UNIT["metre",1.0].....]'
            convfactor = arg3.split("UNIT[")[1].split("]")[0]\
                .split(',')[-1]

            # Now scale ellipsoid to meters:
            a *= float(convfactor)
